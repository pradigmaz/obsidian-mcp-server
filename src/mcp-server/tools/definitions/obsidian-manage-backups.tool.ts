...
export const obsidianManageBackups = tool('obsidian_manage_backups', {
  ...
  input: z.object({
    action: z
      .enum(['list', 'restore'])
      .describe(
        'Action to perform: "list" to see available backups for a note, or "restore" to read a specific backup.',
      ),
    targetPath: z
      .string()
      .optional()
      .describe(
        'For "list": the path of the original note to list backups for (optional, lists all if omitted). For "restore": the exact filename of the backup to restore.',
      ),
  }),
  output: z.object({
    status: z.string().describe('Operation status: success or error.'),
    message: z
      .string()
      .describe('Human-readable backup listing, restored backup content, or recovery guidance.'),
  }),
  ...
  handler: async (args) => {
    ...
    if (args.action === 'list') {
      ...
      if (!args.targetPath) {
        return {
          status: 'error',
          message: 'targetPath (backup filename) is required for restore.',
        };
      }
  ...
  format: (result) => [{ type: 'text', text: `status: ${result.status}\n\n${result.message}` }],
});
