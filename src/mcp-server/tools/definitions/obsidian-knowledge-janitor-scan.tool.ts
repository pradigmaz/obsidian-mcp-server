...
const JanitorScanResultSchema = z
  .object({
    unstructuredNotes: z
      .array(z.string().describe('Vault-relative unstructured note path.'))
      .describe('List of note paths that are missing the OKF minimum `type` field.'),
    scannedCount: z.number().describe('Total number of notes scanned in the targeted area.'),
  })
  .describe('Janitor scan result.');
...
export const obsidianKnowledgeJanitorScan = createKnowledgeProxyTool({
  ...
  input: z.object({
    folder: z
      .string()
      .optional()
      .describe('Optional folder path to restrict the scan. If omitted, scans the entire vault.'),
  }),
